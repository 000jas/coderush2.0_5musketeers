...
export const SolarPanels: React.FC<SolarPanelsProps> = ({ stateRef, sunPosition }) => {
  ...
  useFrame((_state, delta) => {
    ...
    if (s.missionMode === 'Safe Mode') {
      // Safe mode: flat survival orientation
      targetRotationX = 0;
    } else if (s.missionMode === 'Earth Observation') {
      // Observation mode: panels perpendicular to imaging vector
      targetRotationX = Math.PI / 4;
    } else {
      // Normal / downlink: track the Sun vector
  ...
  return (
    <group>
      ...
      <group ref={leftPanelRef} position={[-0.45, 0, 0]}>
        ...
        {/* Solar Panel Wing (offset from axle) */}
        <group position={[-0.325, 0, 0]}>
          ...
          {/* Accent golden border frame for backing */}
