...
export const Camera: React.FC<CameraProps> = ({ stateRef }) => {
  ...
  // Targets based on camera status
  // 1. Extension (translation along Z local axis)
  // 2. Gimbal rotation (scanning effect during imaging, straight down otherwise)
  useFrame((frameState, delta) => {
    ...
    let targetExtension = 0.05; // IDLE standby
    let targetRotationX = 0;
    let targetRotationY = 0;

    if (s.missionMode === 'Safe Mode') {
      targetExtension = -0.05; // Retracted
      targetRotationX = 0;
      targetRotationY = 0;
    } else if (s.cameraStatus === 'Active') {
      targetExtension = 0.15; // Fully extended
      // Simulate scanning pattern (slight slow rocking back and forth)
      targetRotationX = Math.sin(elapsed * 1.5) * 0.15;
      targetRotationY = Math.cos(elapsed * 1.2) * 0.15;
    }

    // Apply extension lerp
    if (tubeRef.current) {
  ...
  return (
    // Camera is mounted on the nadir (bottom) of the satellite body pointing towards Earth
    <group position={[0, -0.22, 0.05]} ref={gimbalRef}>
      {/* 1. Camera Gimbal Mount Base */}
      ...
      {/* 3. Sliding Camera Tube (Animates extension) */}
